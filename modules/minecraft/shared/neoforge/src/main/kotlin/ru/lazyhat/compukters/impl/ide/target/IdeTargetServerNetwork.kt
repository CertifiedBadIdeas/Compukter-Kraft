...
import ru.lazyhat.compukters.impl.compat.compuktersServer
import ru.lazyhat.compukters.impl.network.ServerOperationScope
import java.util.WeakHashMap

@EventBusSubscriber(modid = MOD_ID)
object IdeTargetServerNetwork {
        ...
        val pending =
            transport.operations.submit(player.uuid) {
                transport.terminals.open(
                    ...
                    payload.generation,
                    payload.target,
                    player.compuktersServer().tickCount.toLong(),
                )
            }
        if (pending == null) {
            context.reply(
    ...
    private fun transport(player: ServerPlayer): ServerTransport {
        val server = player.compuktersServer()
