...
internal object IdeClientBootstrap {
    ...
    private fun onScreenKeyPressed(event: ScreenEvent.KeyPressed.Pre) {
        ...
        if (matchesScreenShortcut(event.keyCode, event.scanCode, modifierActive) && open(Minecraft.getInstance())) {
    ...
    ): Boolean = openIde.keyConflictContext.isActive && modifierActive && openIde.matches(keyCode, scanCode)
