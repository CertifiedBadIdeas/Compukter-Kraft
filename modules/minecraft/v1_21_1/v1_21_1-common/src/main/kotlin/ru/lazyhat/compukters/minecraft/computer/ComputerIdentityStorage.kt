...
internal class ComputerIdentityStorage(
    ...
    fun save(output: CompoundTag) {
        output.putLong(HIGH_KEY, identity.highBits)
        output.putLong(LOW_KEY, identity.lowBits)
    }

    fun load(input: CompoundTag?) {
        val high = input?.takeIf { it.contains(HIGH_KEY) }?.getLong(HIGH_KEY)
        val low = input?.takeIf { it.contains(LOW_KEY) }?.getLong(LOW_KEY)
        identity =
